if exists (select 1 from sys.procedures p inner join sys.schemas s on p.schema_id = s.schema_id where p.name = 'ChecklistTemplate_DetailAttachment_InsertUpdate' and s.name = 'dbo')
begin
	drop proc ChecklistTemplate_DetailAttachment_InsertUpdate
end
GO

Create Proc ChecklistTemplate_DetailAttachment_InsertUpdate
