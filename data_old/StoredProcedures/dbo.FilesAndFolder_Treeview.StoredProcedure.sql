if exists (select 1 from sys.procedures p inner join sys.schemas s on p.schema_id = s.schema_id where p.name = 'FilesAndFolder_Treeview' and s.name = 'dbo')
begin
	drop proc FilesAndFolder_Treeview
end
GO
