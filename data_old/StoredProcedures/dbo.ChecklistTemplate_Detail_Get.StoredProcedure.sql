if exists (select 1 from sys.procedures p inner join sys.schemas s on p.schema_id = s.schema_id where p.name = 'ChecklistTemplate_Detail_Get' and s.name = 'dbo')
begin
	drop proc ChecklistTemplate_Detail_Get
end
GO
create proc ChecklistTemplate_Detail_Get
