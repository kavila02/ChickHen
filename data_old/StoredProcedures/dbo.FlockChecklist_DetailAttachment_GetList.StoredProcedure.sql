if exists (select 1 from sys.procedures p inner join sys.schemas s on p.schema_id = s.schema_id where p.name = 'FlockChecklist_DetailAttachment_GetList' and s.name = 'dbo')
begin
	drop proc FlockChecklist_DetailAttachment_GetList
end
GO

create proc FlockChecklist_DetailAttachment_GetList
