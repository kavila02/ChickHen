if exists (select 1 from sys.procedures p inner join sys.schemas s on p.schema_id = s.schema_id where p.name = 'FlockAttachment_InsertUpdate' and s.name = 'dbo')
begin
	drop proc FlockAttachment_InsertUpdate
end
GO

Create Proc FlockAttachment_InsertUpdate
...
if @I_vFlockAttachmentID = 0
begin
	insert into FlockAttachment (
		FlockID
		, AttachmentID
		, AttachmentTypeID
		, UserName
...
begin
	...
	where @I_vFlockAttachmentID = FlockAttachmentID
end