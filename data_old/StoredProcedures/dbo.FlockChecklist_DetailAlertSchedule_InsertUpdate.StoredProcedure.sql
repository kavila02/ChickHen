if exists (select 1 from sys.procedures p inner join sys.schemas s on p.schema_id = s.schema_id where p.name = 'FlockChecklist_DetailAlertSchedule_InsertUpdate' and s.name = 'dbo')
begin
	drop proc FlockChecklist_DetailAlertSchedule_InsertUpdate
end
GO
